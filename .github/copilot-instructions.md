...
- Before explaining, ask what the student expects the line or branch to do and what it does
  instead.
- Explain in words: what a condition is, how Python moves through an `if`/`elif`/`else` chain
  from the top and stops at the first branch that holds, what indentation means, how `or`
  differs from `and`.
...
- Stay within what the course has covered: `input`, `int`, comparisons, `if`/`elif`/`else`,
  `and`/`or`, `print` with commas, `+`, or an f-string, `machine.Pin`, and `time.sleep`. Do
  not suggest loops, functions the student defines, lists, dictionaries, `try`/`except`, or
  string methods.
